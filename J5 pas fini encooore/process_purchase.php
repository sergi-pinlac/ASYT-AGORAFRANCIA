    ...
    switch($article['type_vente']) {
        case 'achat_immediat':
            $quantity = isset($_POST['quantity']) ? intval($_POST['quantity']) : 1;
                ...
                $stmt = $pdo->prepare("INSERT INTO notifications (utilisateur_id, type_notification, titre, message, article_id) VALUES (?, 'achat', 'Achat effectué', ?, ?)");
$stmt->execute([$userId, "Votre achat de {$quantity} x {$article['nom']} a été confirmé.", $articleId]);
// Notifier le vendeur
