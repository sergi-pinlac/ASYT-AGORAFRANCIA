...
<head>
  ...
  <style>
    ...
    /* Effet léger de carte ombrée autour des détails article */
.article-details {
  background: #fffaf0cc; /* léger fond translucide */
  border-radius: 15px;
  padding: 25px;
  box-shadow: 0 10px 20px rgba(168, 126, 65, 0.2);
  transition: box-shadow 0.3s ease;
  max-width: 700px;
  margin: 0 auto;
}

/* Survol : accentuer l'ombre */
.article-details:hover {
  box-shadow: 0 15px 30px rgba(168, 126, 65, 0.4);
}

/* Image article plus douce avec ombre portée */
.article-details img {
  border-radius: 12px;
...
/* Styliser les labels (ex: Catégorie, Type) */
...
/* Bouton retour plus moderne */
.back-button {
  display: inline-block;
  margin-top: 30px;
  padding: 12px 30px;
  background: linear-gradient(45deg, #a87e41, #6b4f1b);
  color: #fff;
  font-weight: 700;
  border-radius: 25px;
  border: none;
  box-shadow: 0 5px 10px rgba(168,126,65,0.4);
  text-decoration: none;
  transition: background 0.4s ease, box-shadow 0.4s ease;
}

.back-button:hover {
  background: linear-gradient(45deg, #6b4f1b, #a87e41);
  box-shadow: 0 8px 20px rgba(168,126,65,0.6);
  color: #fff;
}

/* Rendre le texte description un peu plus lisible */
.article-details p {
  line-height: 1.6;
  font-size: 18px;
  color: #4a3d1a;
...
/* Responsive : centrer la page sur mobile */
